  ...
  useEffect(() => {
    const fetchUserRooms = async () => {
      try {
        ...
        console.log(response.data.userRooms)
        setUserRooms(response.data.userRooms); // Assuming the server returns the user rooms as an array
...
//      setMessages(messages.messages)
  ...
  const copyRoomIdToClipboard=(roomId)=>{
    ...
    .then(() => {
        // Notify the user that the roomId has been copied successfully
...
{userRooms.map(
    (room,index)=>{
        return <div key={room.roomId} className="border-b-black border-2 px-1 py-1 bg-white justify-center  ">
        <div> 
